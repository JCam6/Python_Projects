...
def main():
    ...
    background_netlist = os.system('nmap -O -oG networklist.txt "%s" | grep -v "OS:\|TCP"' % (fullrange_gateway_IP))
    print("\n" + "*" * 10 + "A list of network hosts was saved in networklist.txt." + "*" * 10)
    ...
    while repeat == "yes":
        ...
        print("\n")
        print("*" * 10 + "Open this link to find potential OS weakesses to fix." + "*" * 10 + "\n")
        ...
        # Run scan of network.
        os.system('nmap -sn "%s" | grep "report" | cut -d " " -f 5' % (fullrange_gateway_IP))
